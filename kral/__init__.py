    ...
    if args.parser == 'stream':
        ...
        for item in stream(args.queries, args.services):
            ...
            try:
                ...
                print u"{0:7d} | {1:8s} | {2:8s} | {3:22s} | {4:5d}| {5:22s}| {6:22s}| {7:22s} | {8:140s}".format(count,item['service'], item['query'], item['user']['name'], item['user']['subscribers'], item['id'], item['user']['location'], item['coordinates'], item['text'].replace('\n','').encode('unicode-escape'))
            except:
                pass
